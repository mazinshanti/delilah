...
export function normalizeInventoryListing(input = {}, context = {}) {
  ...
  const priceSar = ['installment','contact'].includes(input.price_type)?null:numberOrNull(input.price_sar ?? input.priceSar ?? input.price);
  ...
  const images = vehicleImages({...input,images:input.gallery_images||input.images,image:input.primary_image||input.image});
