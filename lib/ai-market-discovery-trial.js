...
export async function runAdaptiveMarketDiscovery(body,intent,{discover=discoverMarketWithAI,readDetail=createMarketDetailReader(),maxRounds=3,maxDetails=24,onProgress=()=>{}}={}){
 ...
 const started=performance.now(),feedback=[],seen=new Set(),pending=new Map(),discoveredUrls=new Set(),seenPages=new Set(),pageResults=[],results=[],listings=[];let webSearchCalls=0,firstResultMs=null,stopReason='round-budget';
 ...
 for(const raw of candidates){const c=marketCandidate(raw);if(!c||discoveredUrls.has(c.url))continue;discoveredUrls.add(c.url);pending.set(c.url,c);}
