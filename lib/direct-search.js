...
function matchesRequestedVehicle(car={},query=''){
  ...
  if(model&&declaredModel&&declaredModel!==model)return false;
...
export function strictDirectListings(listings=[],body={}){
  ...
  let xs=filterVehicleSaleListings(listings).filter(car=>car?.condition===condition);
  ...
  if(filters.model)xs=xs.filter(car=>listingMatchesModel(car,filters.model));
  if(filters.category)xs=xs.filter(car=>/^(electric|hybrid)$/i.test(filters.category)?norm(car.fuelType||car.vehicle?.fuelType||'')===norm(filters.category):norm(car.bodyType||car.vehicle?.bodyType||'')===norm(filters.category));
