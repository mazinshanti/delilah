...
export const MODEL_GROUPS={
  k3:['k3','كي 3'],
  'corolla cross':['corolla cross','كورولا كروس'],
...
export function detectRequestedModel(query=''){
 ...
 const intent=catalogIntent(query),hay=` ${q} `,model=MODEL_ALIASES.find(a=>hay.includes(a.alias))?.model||intent.modelKey||null;
...
export function listingMatchesModel(car={},model=null){
 ...
 if(model.includes('::'))return catalogModelMatches(car,model);
