...
export class InventoryIndex{
 ...
 replace(data){this.records=deduplicateVehicles(data.listings||[]).sort((a,b)=>Number(b.dataComplete)-Number(a.dataComplete)||Number(Boolean(b.image))-Number(Boolean(a.image)));this.generatedAt=data.generatedAt||new Date().toISOString();this.diagnostics=data.diagnostics||[];}
