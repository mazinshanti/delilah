...
let listings=Array.from({length:size},(_,i)=>{const c=real[i%real.length],url=new URL(c.url);url.searchParams.set('capacity-fixture',String(i));return {...c,url:url.href,originalUrl:url.href,source_url:url.href,id:'capacity-'+i,inventoryId:'capacity-'+i,vin:null,trim:null,lastSeenAt:now};});
// Round-trip to approximate independently loaded records rather than shared fixture strings.
listings=JSON.parse(JSON.stringify(listings));global.gc?.();
const start=performance.now(),idx=new InventoryIndex();idx.replace({generatedAt:now,listings});const loadedMs=performance.now()-start;listings=null;global.gc?.();
