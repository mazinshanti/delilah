...
const cases=[{id:'corolla-en',query:'Toyota Corolla under 45000 in Jeddah',condition:'used'},{id:'suv-ar',query:'ابي جيب عائلي ياباني تحت 150 ألف في الرياض',condition:'used'},{id:'new-ar',query:'تويوتا كورولا جديدة تحت 100 ألف',condition:'new'}];
...
const report={at:new Date().toISOString(),productionChanged:false,inventoryLoaded:false,imagesDownloaded:0,hostedDiscoveryConfigured:Boolean(process.env.OPENAI_API_KEY||process.env.TAVILY_API_KEY),cases:[]};
for(const c of cases.filter(c=>!process.env.TRIAL_CASE||c.id===process.env.TRIAL_CASE)){
 ...
 const result=await runSearch(baseline.intent,{...adapters,cache,onResult:rows=>{events.push({count:rows.length});console.log(JSON.stringify({case:c.id,event:'verified-results',count:rows.length}));}});
 let warm=null;
