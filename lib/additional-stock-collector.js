...
import {createStockFrontier,sitemapLocations,allowedStockSitemap,allowedStockDiscoveryPage} from './stock-frontier.js';
...
export async function collectAdditionalStock({fetchImpl=fetch,mediaMode='validate',expandNavigation=false,maxPages=100,maxDetails=2500,maxSitemaps=25,maxDurationMs=30*60000,concurrency=3,wait=sleep,onProgress=()=>{},onCheckpoint=async()=>{},sources=ADDITIONAL_MARKET_SOURCES,previousDiagnostics=[],previousListings=[],state={},now=Date.now}={}){
 ...
 if(!['validate','links-only'].includes(mediaMode))throw Error('invalid-media-mode');
 const validateMedia=mediaMode==='validate'?stockMediaValidator(fetchImpl):async record=>({...record,imageVerified:false,mediaValidation:'source-url-only'});
 const results=await Promise.all(sources.map(async source=>{
  ...
  if(Number(saved.pauseUntil)>started)return {listings:[],removedUrls:[],state:saved,diagnostics:{source:source.name,records:0,paused:true,pauseUntil:saved.pauseUntil,coverageComplete:false}};
  let pauseUntil=0;
  const priorById=new Map(previousListings.filter(c=>c.source===source.name).map(c=>[stockIdentity(source,c.url),c]));
  ...
  const validPage=raw=>{try{const u=new URL(raw);return expandNavigation?allowedStockDiscoveryPage(source,raw):u.origin===entry.origin&&u.pathname===entry.pathname&&!u.username&&!u.password;}catch{return false;}};
  ...
  const entryDue=(expandNavigation&&!saved.navigationExpanded)||!saved.entryCheckedAt||started-Date.parse(saved.entryCheckedAt)>=30*60000;
  ...
  const savedState=()=>({...frontier.save(),navigationExpanded:expandNavigation||saved.navigationExpanded||false,pauseUntil,pageQueue:[...queue],sitemapQueue:[...maps],visitedPages:[...pages],visitedSitemaps:[...mapSeen],entryCheckedAt,discoveryAt:rescan?new Date(started).toISOString():saved.discoveryAt||new Date(started).toISOString()});
  const checkpoint=()=>onCheckpoint({sourceId:source.id,listings:[...listings],removedUrls:[...removedUrls],diagnostics:{...d,...frontier.stats()},state:savedState()});
  ...
  async function get(url){for(let redirects=0;redirects<4;redirects++){
   ...
   if(!r.ok){await r.body?.cancel();if([401,403,429].includes(r.status)){stop=true;const retry=Number(r.headers.get('retry-after'));pauseUntil=now()+Math.max(3600000,Math.min(86400000,Number.isFinite(retry)?retry*1000:0));}throw Error((redirects>0&&[404,410].includes(r.status)?'redirect-target HTTP ':'HTTP ')+r.status);}return {html:await readBody(r),url:u.href};
  ...
  async function discovery(){
   ...
   if(maps.length&&d.sitemaps<maxSitemaps&&!(entryDue&&d.pages===0&&queue.includes(entry.href))){const url=maps.shift();if(mapSeen.has(url))return;d.sitemaps++;
    ...
    catch(e){d.errors.push({stage:'sitemap',url,error:e.message.slice(0,150)});if(/^HTTP (404|410)$/.test(e.message))mapSeen.add(url);else{maps.unshift(url);stop=true;}}
    ...
    try{const {html,url:pageUrl}=await get(page);d.successfulPages++;pages.add(page);if(page===entry.href)entryCheckedAt=new Date(now()).toISOString();stockLinks(html,source).forEach(discover);
     for(const m of html.matchAll(/<a\b[^>]*href=["']([^"']+)["'][^>]*>/gi)){try{const u=new URL(m[1].replace(/&amp;/g,'&'),pageUrl),p=new URL(pageUrl),n=Number(u.searchParams.get('page'));const nextPage=Number.isInteger(n)&&n>Number(p.searchParams.get('page')||1)&&n<=10000;const navigation=expandNavigation&&u.pathname!==p.pathname&&!u.search; if(validPage(u.href)&&(nextPage||navigation)&&!pages.has(u.href)&&!queue.includes(u.href)){if(queue.length<10000)queue.push(u.href);else d.pageQueueOverflow=(d.pageQueueOverflow||0)+1;}}catch{}}
  ...
  async function detail(e){d.detailAttempts++;processed.add(stockIdentity(source,e.url));try{
   ...
   for(const record of valid)listings.push(await validateMedia({...record,detailChecked:true,lastDetailAt:new Date(now()).toISOString(),evidenceLevel:'exact-ad-source-adapter'}));
   const outcome=valid.length?'accepted':parsed.reason||'unknown-condition';const old=priorById.get(stockIdentity(source,e.url));const changed=Boolean(old&&valid.some(c=>['price','mileage','condition','city'].some(k=>c[k]!==old[k])));frontier.finish(e.url,outcome,now(),{changed,popular:old?.refreshPriority==='high'});if(!valid.length){d.rejectedReasons[outcome]=(d.rejectedReasons[outcome]||0)+1;if(outcome==='unavailable')removedUrls.push(e.url);}
  ...
  try{
   const r=await response(new URL('/robots.txt',source.url).href);if(r.ok)robots=await readBody(r);else if(r.status===404)robots='';else{if([401,403,429].includes(r.status))pauseUntil=now()+3600000;throw Error('robots-unavailable '+r.status);}
  ...
  await checkpoint();
  return {listings,removedUrls,diagnostics:d,state:savedState()};
