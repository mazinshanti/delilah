...
export function queryTerms(q = '') {
  const tokens = canonicalText(q).split(' ').filter(Boolean), out = [];
  for (let i = 0; i < tokens.length; i++) {
    const t = tokens[i];
    if (STOP.has(t) || /^(?:19|20)\d{2}$/.test(t) || /^\d+(?:\.\d+)?k$/.test(t)) continue;
    if (/^\d{1,3}$/.test(t)) {
      const prev = tokens[i - 1] || '', next = tokens[i + 1] || '';
      const modelAdjacent = (prev && !STOP.has(prev) && /^[a-z][a-z0-9]{0,12}$/.test(prev)) || next === 'series';
      if (modelAdjacent) out.push(t);
      continue;
    }
    if (t.length >= 2 && !/^\d+(?:\.\d+)?$/.test(t)) out.push(t);
  }
  return [...new Set(out)].slice(0, 7);
}
export function identityMatches(c, q) {
  const terms = queryTerms(q); if (!terms.length) return true;
  const tokens = new Set(canonicalText(`${c.brand || ''} ${c.model || ''} ${c.title || ''} ${c.snippet || ''}`).split(' ').filter(Boolean));
  return terms.every(t => tokens.has(t));
}
