...
export function marketDiscoveryPage(raw){
 ...
 const valid=(source.id==='carswitch'&&/^\/(?:en\/)?[^/]+\/(?:used-cars|حراج-السيارات)(?:\/[^/]+){0,3}\/?$/.test(path))
  ||(source.id==='saudisale'&&/^\/(?:en\/)?car-classes\/\d+\/[^/]+\/listings\/?$/.test(path))
  ||(source.id==='syarah'&&/^\/en\/autos\/?$/.test(path));
 if(!valid)return null;u.hash='';u.search='';return {url:u.href,source,discoveryPage:true};
 }catch{return null;}
}
...
export async function runAdaptiveMarketDiscovery(body,intent,{discover=discoverMarketWithAI,readDetail=createMarketDetailReader(),maxRounds=3,maxDetails=24,onProgress=()=>{}}={}){
 ...
 const feedback=[],seen=new Set(),seenPages=new Set(),pageResults=[],results=[],listings=[];let webSearchCalls=0,stopReason='round-budget';
 ...
 for(const raw of candidates){const c=marketCandidate(raw);if(!c||seen.has(c.url))continue;
 ...
 return {mode:'adaptive-multi-source-trial',sources:AI_MARKET_SOURCES.map(s=>s.name),webSearchCalls,discoveryPages:pageResults,discovered:seen.size,checked:results.length,accepted:listings.length,listings:mergeDirectListings(listings),results,rounds:feedback.length,stopReason,coverageComplete:false};
}
