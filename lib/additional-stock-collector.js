...
export async function collectAdditionalStock({fetchImpl=fetch,maxPages=100,maxDetails=2500,maxSitemaps=25,maxDurationMs=30*60000,concurrency=3,wait=sleep,onProgress=()=>{},onCheckpoint=async()=>{},sources=ADDITIONAL_MARKET_SOURCES,previousDiagnostics=[],previousListings=[],state={},now=Date.now}={}){
 ...
 const results=await Promise.all(sources.map(async source=>{
  ...
  const savedState=()=>({...frontier.save(),pageQueue:[...queue],sitemapQueue:[...maps],visitedPages:[...pages],visitedSitemaps:[...mapSeen],entryCheckedAt,discoveryAt:rescan?new Date(started).toISOString():saved.discoveryAt||new Date(started).toISOString()});
