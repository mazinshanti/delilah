...
export function inventoryRecord(raw,source){
  ...
  const make=detectRequestedBrand(raw.make)||raw.make||detectRequestedBrand(title),model=raw.model||detectRequestedModel(title);
...
export function parseSyarahInventory(html,source){
  ...
  return (data?.posts||[]).flatMap(p=>{
    ...
    const r=inventoryRecord({url:p.product_url,title:p.title,make:g.post_make,model:g.post_model,year:g.post_year,trim:g.post_ext,price:p.sellingprice,mileage:g.post_mileage,condition:String(g.post_condition||'').toLowerCase(),city:g.post_city,image:p.image_url,fuelType:g.post_fuel,transmission:g.post_transmission},source);
