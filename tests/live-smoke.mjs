...
for(const t of cases){
 try{
  const r=await fetch(`${base}/api/search`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({query:t.q,condition:t.condition,filters:t.filters||{}}),signal:timeout(100000)});
  if(!r.ok)throw new Error(`HTTP ${r.status}: ${(await r.text()).slice(0,200)}`);
  const d=await r.json();
  if(!Array.isArray(d.listings))throw new Error('listings not array');
  if(t.min&&d.listings.length<t.min)throw new Error(`expected at least ${t.min} listings, got ${d.listings.length}`);
  ...
  for(const c of d.listings){
   ...
   if(c.source==='Motory'&&!/^https:\/\/ksa\.motory\.com\/en\/cars-for-sale\/[^/]+\/[^/]+\/[^/]+\/20\d{2}\/\d+\/?$/i.test(c.url))throw new Error(`bad Motory listing url ${c.url}`);
   ...
   if(c.image){
    if(c.imageVerified!==true||c.imageSource!=='listing_page')throw new Error(`unverified image: ${c.url}`);
  ...
  console.log(`PASS ${t.condition.padEnd(4)} | ${String(d.listings.length).padStart(2)} cars | ${String(images).padStart(2)} images | ${t.filters?.seller||'all'} | ${t.q}`);pass++;
 }catch(e){console.error(`FAIL ${t.condition} | ${t.q} | ${e.message}`);fail++}
}
console.log(`live smoke: ${pass} passed, ${fail} failed; ${totalCars} cars; ${totalImages} exact-page images; relay ${relayPass}/${relayChecks}`);
