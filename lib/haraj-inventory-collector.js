...
export function harajDetailRecord(candidate,html,now=new Date().toISOString(),onReject=()=>{}){
 ...
 const years=new Set([...titleDigits.matchAll(/\b((?:19|20)\d{2})\b/g),...descriptionDigits.matchAll(/(?:^|\n)\s*(?:الموديل|موديل|سنة الصنع|model year)\s*[:：-]?\s*((?:19|20)\d{2})\b/gi)].map(m=>Number(m[1])));
