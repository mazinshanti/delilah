...
export async function collectHarajInventory(options={}){
 ...
 const started=Date.now(),now=new Date().toISOString(),listings=[],seen=new Set(),errors=[],pendingCandidates=[];
 ...
 const resume=[...new Map((options.pendingCandidates||[]).filter(c=>adKey(c.url)).map(c=>[adKey(c.url),c])).values()];
  ...
  try{
   ...
   async function worker(){
    while(cursor<targets.length&&!stop&&diag.detailAttempts<maxDetails&&Date.now()<deadline){
     ...
     }catch(e){errors.push({stage:'detail',query,error:e.message});if(/HTTP (401|403|429)|robots-disallowed|unsafe-source-redirect/.test(e.message))stop=true;}
    }
   }
