...
export function installOnDemandRoutes(app,{env=process.env,intentEngine,service:injected,authorizeSession=()=>false}={}){
 ...
 const authorize=(req,res,next)=>{res.setHeader('Cache-Control','no-store');if(authorizeSession(req))return next();const actual=Buffer.from(req.get('authorization')||''),expected=Buffer.from('Bearer '+token);if(actual.length!==expected.length||!timingSafeEqual(actual,expected))return res.status(401).json({error:'unauthorized'});next();};
