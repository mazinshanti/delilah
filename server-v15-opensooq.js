...
function attach(d={},j){if(!j)return d;const lower=Array.isArray(d.listings)?d.listings:[],listings=mix(lower,j.listings,j.broad),lowerComplete=d.complete===true||d.marketScanComplete===true,complete=lowerComplete&&j.complete;return{...d,listings,counts:counts(listings),complete,marketScanComplete:complete,opensooqStructuredInventory:true,opensooqComplete:j.complete,opensooqListings:listings.filter(x=>x.source==='OpenSooq').length,opensooqFetchedListings:j.listings.length,opensooqErrors:j.errors.slice(0,8),opensooqRoot:j.root,opensooqBroadQuota:BROAD_OPENSOOQ_QUOTA,opensooqSourceMix:j.broad}}
...
app.get('/api/health',async(req,res)=>{try{const{r,d}=await inner('/api/health',{signal:AbortSignal.timeout(9000)});return res.status(r.status).json({...d,edge:'dalelah-v15-volume',productVersion:'1.5',renderGitCommit:process.env.RENDER_GIT_COMMIT||d.renderGitCommit||null,opensooqStructuredInventory:true,opensooqBroadQuota:BROAD_OPENSOOQ_QUOTA,opensooqNativeExact:true})}catch(e){return res.status(503).json({ok:false,edge:'dalelah-v15-volume',productVersion:'1.5',renderGitCommit:process.env.RENDER_GIT_COMMIT||null,opensooqStructuredInventory:true,error:e?.message||String(e)})}});
