...
export async function discoverPublicCarListings({query,intent,apiKey,model="gpt-5.6-luna",timeout=15000,maxCandidates=8}){
  ...
  const prompt=`Search the public web for current Saudi Arabia WHOLE CAR listings matching: ${query}. Find direct vehicle-for-sale pages only. Prioritize Haraj, Syarah, Saudi Sale, ArabWheels, YallaMotor, CarSwitch, Motory, official dealers and certified-used dealers. If make/model/year/city are given, search them literally. Exclude parts, wanted ads, category/search pages, reviews and price/spec pages. Never invent a URL. ${internetResearchRules()}`;
