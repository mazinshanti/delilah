...
export function createPreviewApp({env=process.env,intentEngine,service}={}){
 ...
 app.get('/health',(_req,res)=>res.json({ok:true,preview:true,commit:env.RENDER_GIT_COMMIT||null,node:process.version,mode:env.DALELAH_ON_DEMAND_PROVIDER,configured:{brave:Boolean(env.BRAVE_SEARCH_API_KEY||env.BRAVE_API_KEY),openai:Boolean(env.OPENAI_API_KEY),tavily:Boolean(env.TAVILY_API_KEY)}}));
 ...
 app.get('/api/sources',(_req,res)=>res.json({sources:allSources.map(name=>({name,status:'connected-live'}))}));
