...
const selected=['saudisale','motory','samaco','arabwheels'];
...
function state(transport){if(!states.has(transport))states.set(transport,{robots:new Map(),paused:new Set(),queues:new Map()});return states.get(transport);}
async function get(transport,url,signal,priority=0){
 const st=state(transport),origin=new URL(url).origin;
 if(st.paused.has(origin))throw Error('source-paused');
 ...
 const task=previous.catch(()=>{}).then(async()=>{
  if(signal?.aborted)throw Error('deadline');if(st.paused.has(origin))throw Error('source-paused');
  ...
  try{return await transport.get(url,{signal,priority,permit:target=>robotsPolicy(robots,target).allowed});}catch(e){if(/http-(401|403|429)|access-blocked/.test(e.message))st.paused.add(origin);throw e;}
...
export async function additionalDiscovery(i,{transport,signal,onBatch,diagnostics}){
 const tasks=EXPANDED_SOURCES.filter(s=>s.id!=='samaco'||!i.make||['Audi','Volkswagen','Porsche','Bentley','Lamborghini','Skoda'].includes(i.make)).filter(s=>s.id!=='arabwheels'||i.condition!=='new').map(async source=>{
