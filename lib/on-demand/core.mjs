import {mstamlIdentity} from '../mstaml-detail-trial.js';
...
export function identity(raw){try{const u=new URL(raw);if(u.protocol!=='https:'||u.username||u.password||u.port)return null;
 if(u.hostname==='www.mstaml.com'){const id=mstamlIdentity(raw);return id?u.hostname+':'+id:null;}
 const extra={
 'sa.opensooq.com':/^\/en\/search\/(\d{6,})\/?$/,
 'www.mercedes-benz-mena.com':/^\/ksa\/en\/buy-used\/(\d+)-[^/]+\/?$/,
 'buy.kayishha.com':/^\/(?:ar\/|en\/)?cars\/details\/[^/]+-(\d{4,})\/?$/,
 'cars.saudisale.com':/^\/(?:index\.php\/)?(?:en\/)?listings\/([a-zA-Z0-9]+)\/[^/]+\/?$/,
 'ksa.motory.com':/^\/(?:en|ar)\/cars-for-sale\/[^/]+\/[^/]+\/[^/]+\/\d{4}\/(\d+)\/?$/,
 'www.samaco.com.sa':/^\/(?:en\/)?stock\/(?:vehicle\/)?(\d{6,})-[^/]+\/?$/,
 'www.arabwheels.sa':/^\/used-cars\/[^/]+-for-sale-in-[^/]+-(\d+)$/
 };
 const id=extra[u.hostname]?u.pathname.match(extra[u.hostname])?.[1]:u.hostname==='haraj.com.sa'?u.pathname.match(/^\/(\d{8,})/ )?.[1]:u.hostname==='ksa.carswitch.com'?u.pathname.match(/\/used-car\/[^/]+\/[^/]+\/\d{4}\/(\d+)/)?.[1]:u.hostname==='www.salehcars.com'?u.pathname.match(/\/cars\/([a-f0-9]{24})/)?.[1]:u.hostname==='syarah.com'?u.pathname.match(/\/cardetail\/[^/]+-(\d+)/)?.[1]:null;
...
// Spend the bounded detail budget on identifiable matches before unknown cards.
export function candidatePriority(c,i){
 const cat=catalogIntent([c.make||c.brand,c.model,c.title].filter(Boolean).join(' '));
 return (i.make&&cat.make&&compact(cat.make)===compact(i.make)?4:0)+(i.model&&cat.model&&compact(cat.model)===compact(i.model)?8:0)+(i.city&&c.city&&compact(c.city)===compact(i.city)?2:0)+(c.condition===i.condition?1:0);
}
...
export async function runSearch(intent,{discover,verify,cache=new ResultCache(),deadlineMs=45000,maxDetails=12,detailConcurrency=4,onResult=()=>{},signal,clock=Date.now}={}){
 ...
 const started=clock(),controller=new AbortController();const abort=()=>controller.abort();if(signal?.aborted)abort();else signal?.addEventListener('abort',abort,{once:true});const timer=setTimeout(()=>controller.abort(),deadlineMs),diagnostics=[],seen=new Set(),verified=[];
 ...
 async function worker(){while(!controller.signal.aborted){
  ...
  // Keep source fairness, then prefer identified matches within the available budget.
  queue.sort((a,b)=>(perSource.get(a.source)||0)-(perSource.get(b.source)||0)||candidatePriority(b,intent)-candidatePriority(a,intent));
 ...
 try{await Promise.all([discovery,...Array.from({length:detailConcurrency},worker)]);}finally{clearTimeout(timer);signal?.removeEventListener('abort',abort);}
