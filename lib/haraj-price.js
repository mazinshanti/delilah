...
function unsafeAmount(input,m){
 ...
 return contextLooksLikeMileage(input,start,end)||/^\s*(?:د\.\s*د|rpm\b|cc\b|hp\b|حصان|سلندر)/i.test(after)||/(?:راتب|salary|engine|محرك|حصان)[^.;،]{0,30}$/i.test(before);
}
...
export function extractHarajPrice(text='',opts={}){
  ...
  if(opts.allowFormatted===false)return null;
