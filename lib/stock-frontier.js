...
// Only source-published navigation links are passed here; no synthetic brand URLs.
export function allowedStockDiscoveryPage(source,raw){
 try{const u=new URL(raw),entry=new URL(source.path,source.url);
  if(u.protocol!=='https:'||u.origin!==entry.origin||u.username||u.password||u.hash||stockIdentity(source,u.href))return false;
  if(u.pathname===entry.pathname)return true;
  if([...u.searchParams.keys()].some(k=>k!=='page'))return false;
  if(source.id==='motory')return /^\/(?:en|ar)\/cars-for-sale\/(?:[a-z0-9-]+\/){1,3}$/.test(u.pathname);
  if(source.id==='arabwheels')return /^\/used-cars\/search\/(?:[a-z0-9_-]+\/){1,4}$/.test(u.pathname);
  return false;
 }catch{return false;}
}
