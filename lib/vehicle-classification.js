...
export function classifyVehicle(car={}){
 ...
 // A clock in explicit opening-hours text is not a watch advertisement.
 const nonAutoDescription=description.replace(/من الساعه \d{1,2}(?: \d{2})? (?=صباحا|مساء|ظهرا|عصرا)/g,' ');
 const result=(classification,reason)=>({classification,reason,evidence,identity,version:CLASSIFICATION_VERSION});
 if(/^(?:for sale\s+)?horse(?:\s|$)/i.test(title)||/\bhorse\b/i.test(category)||NON_AUTO.test(category)||NON_AUTO.test(title)||NON_AUTO.test(nonAutoDescription))return result('NON_AUTOMOTIVE','non_automotive_subject');
 // A standalone descriptive clause (e.g. sedan in demand) is not a purchase request.
 const wantedDescription=catalogText(String(car.description||car.snippet||'').replace(/(^|\n)(\s*(?:سيارة|سياره|سيدان)\s+(?:فاخرة|فاخره))\s+مطلوب[ةه](?=\s*(?:[|؛]|\n|$))/g,'$1$2'));
 const wantedContext=wantedDescription.replace(/(?:لا يوجد|ليس هناك|ما فيه)\s+مطلوب(?:ات)?\s+عل[ىي]\s+(?:السياره|السيارة)/g,' ');
 if(WANTED.test(title)||WANTED.test(category)||WANTED.test(wantedContext))return result('WANTED_VEHICLE','wanted_ad');
...
const metrics={};
export function recordClassification(source,verdict){const key=String(source||'Unknown');const m=metrics[key]||={ingested:0,classes:{},reasons:{},identityFailures:0};m.ingested++;m.classes[verdict.classification]=(m.classes[verdict.classification]||0)+1;m.reasons[verdict.reason]=(m.reasons[verdict.reason]||0)+1;if(!verdict.identity.make||!verdict.identity.model)m.identityFailures++;}
