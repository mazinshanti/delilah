...
export function parseMstamlDetail(html,candidate){
 ...
 if(/(?:إيجار|ايجار|تأجير|تاجير|rent(?:al)?|lease)/i.test(description))return fail('rental-or-lease');
 const labelMileage=number(fields.get('ممشى السيارة بالكيلو متر')),descriptionMileage=extractMileage(description);
