import {extractSalehVehicleImage,extractSalehVehicleGallery} from './saleh-image.js';
import {extractSalehPrice} from './saleh-price.js';
import {catalogMake} from '../public/catalog.js';
...
export function detectSalehIntent(query=''){
  ...
  const make=catalogMake(query);
  if(make)return{key:make.name.toLowerCase(),aliases:[],brand:[make.name,make.ar,...make.aliases].filter(Boolean),year:years.length===1?years[0]:null,brandOnly:true};
  return null;
}
...
function candidateMatches(url='',meta={}){
  ...
  if(!meta.brandOnly&&!meta.aliases.some(a=>text.includes(norm(a))))return false;
...
// Warm the public product index without delaying startup or the first useful
// cached inventory response. Failures remain non-fatal and retry on demand.
export function prewarmSalehInventory(){return liveSalehUrls(20_000).catch(()=>[])}
...
async function fetchOne(url,meta,timeout){
  try{
    ...
    const title=h1?strip(h1):text.slice(0,180);
    const matchText=norm(`${title} ${decodeURIComponent(r.url||url)}`);
    const years=[...digits(`${title} ${decodeURIComponent(r.url||url)}`).matchAll(/\b(20\d{2})\b/g)].map(x=>Number(x[1]));
    const year=years.find(y=>y>=2000&&y<=2035)||null;
    if(meta.year&&year!==meta.year)return null;
    if(!meta.brandOnly&&!meta.aliases.some(a=>matchText.includes(norm(a))))return null;
    ...
    const priceHit=extractSalehPrice(text.slice(0,9000),html);
    const price=priceHit?.price??null;
    const images=extractSalehVehicleGallery(html,r.url||url,{title});const image=images[0]||null;
    return{
      source:'Saleh Cars',sourceType:'dealer',seller:'Saleh Cars',sourceStrict:true,
      title,snippet:text.slice(0,600),url:r.url||url,salehProductId:salehProductId(r.url||url),
      year,price,mileage:null,city:null,condition:'new',saleVerified:true,
      saleEvidence:['saleh_live_sitemap','saleh_verified_product_page'],
      images,galleryVerified:images.length>0,image:image||null,displayImage:image||null,imageVerified:Boolean(image),
      priceVerified:Boolean(price),priceSource:priceHit?.source??null,priceEvidence:priceHit?.evidence??null,
      salehVatPrice:priceHit?.vatPrice??null,
      score:99,discovery:'saleh_live_sitemap',fastSource:true,salehLiveInventory:true
    };
  }catch{return null}
}
...
export async function fetchSalehFast({query='',filters={},timeout=2200}={}){
  ...
  const meta=detectSalehIntent(query);
  if(!meta)return{listings:[],error:'unsupported-fast-model'};
  const started=Date.now();
  let urls=[];
  try{urls=await liveSalehUrls(Math.min(4500,timeout));}
  catch(error){return{listings:[],error:error?.message||String(error),model:meta.key,liveInventory:false};}
  const candidates=urls.filter(url=>candidateMatches(url,meta)).slice(0,meta.brandOnly?16:12);
