...
async function getPool() {
  ...
  if (!poolPromise) {
    poolPromise = import('pg').then(({Pool}) => new Pool({
      ...
      ssl: String(process.env.SAUDI_DATABASE_SSL || 'true').toLowerCase() === 'false' ? false : { rejectUnauthorized: true, ...(process.env.SAUDI_DATABASE_CA ? {ca:process.env.SAUDI_DATABASE_CA.replace(/\\n/g,'\n')} : {}) },
