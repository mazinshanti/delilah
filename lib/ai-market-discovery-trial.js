import {MSTAML_TRIAL_SOURCE,mstamlIdentity,parseMstamlDetail} from './mstaml-detail-trial.js';
...
// Existing connected sources plus isolated trial adapters; not a production activation list.
export const AI_MARKET_SOURCES=[...SOURCE_REGISTRY.filter(s=>/^connected-/.test(s.status)&&s.detailPattern),MSTAML_TRIAL_SOURCE];
export function marketCandidate(raw){
 ...
 if(!source)return null;if(source.id==='mstaml'&&!mstamlIdentity(u.href))return null;u.hash='';if(source.id!=='mstaml')u.search='';else for(const key of [...u.searchParams.keys()])if(/^utm_/i.test(key)||['gclid','fbclid'].includes(key))u.searchParams.delete(key);if(source.id==='haraj')u.pathname=`/${u.pathname.split('/')[1]}/`;
...
export function marketListingKey(raw){
 ...
 if(c.source.id==='mstaml')return `mstaml:${mstamlIdentity(c.url)}`;
...
export function marketDiscoveryPage(raw){
 ...
 const valid=(source.id==='saudisale'&&/^\/en\/?$/.test(path))
  ...
  ||(source.id==='haraj'&&/^\/(?:en\/)?users\/[^/]{1,160}\/?$/.test(path))
  ||(source.id==='opensooq'&&/^\/en(?:\/[^/]+)?\/cars\/cars-for-sale(?:\/[^/]+){0,3}\/?$/.test(path))
...
export function createMarketDetailReader({fetchImpl=fetch,sleep=ms=>new Promise(r=>setTimeout(r,ms)),clock=Date.now,detailTtlMs=60000,pageTtlMs=15000,maxCacheBytes=20000000}={}){
 const responses=new Map(),inflight=new Map();let cacheBytes=0;
 const forget=key=>{const old=responses.get(key);if(old){cacheBytes-=old.bytes;responses.delete(key);}};
 const policies=new Map(),blocked=new Map(),queues=new Map();
 ...
 if([301,302,303,307,308].includes(r.status)&&context){
  ...
  const policy=robotsPolicy(policies.get(target.origin).text,target.href);
 ...
 const origin=new URL(checked.url).origin;if(blocked.has(origin)){if(clock()<blocked.get(origin))throw Error('source-paused');blocked.delete(origin);}
 try{
 if(!policies.has(origin)||clock()>=policies.get(origin).expiresAt)policies.set(origin,{text:await read(`${origin}/robots.txt`),expiresAt:clock()+600000});
 const p=robotsPolicy(policies.get(origin).text,checked.url);if(!p.allowed)throw Error('robots-disallowed');if(p.delayMs>30000)throw Error('source-delay-exceeds-budget');
 await sleep(p.delayMs);return await read(checked.url,checked);
 }catch(e){if(/HTTP (401|403|429)/.test(e.message))blocked.set(origin,clock()+600000);throw e;}
 };
 return candidate=>{
 ...
 const key=checked.url,cached=responses.get(key);
 if(cached){if(clock()<cached.expiresAt)return Promise.resolve(cached.html);forget(key);}
 // Simultaneous identical requests share one paced source read. Query filters
 // remain outside this transport and are reapplied to the parsed records.
 if(inflight.has(key))return inflight.get(key);
 const origin=new URL(checked.url).origin,previous=queues.get(origin)||Promise.resolve();
 const job=previous.catch(()=>{}).then(()=>fetchCandidate(checked)).then(html=>{
  const ttl=checked.discoveryPage?pageTtlMs:detailTtlMs,bytes=Buffer.byteLength(html);
  for(const [k,v]of responses)if(clock()>=v.expiresAt)forget(k);
  if(ttl>0&&bytes<=maxCacheBytes){while(cacheBytes+bytes>maxCacheBytes&&responses.size)forget(responses.keys().next().value);responses.set(key,{html,bytes,expiresAt:clock()+ttl});cacheBytes+=bytes;}
  return html;
 });queues.set(origin,job);inflight.set(key,job);
 job.finally(()=>{inflight.delete(key);if(queues.get(origin)===job)queues.delete(origin);}).catch(()=>{});return job;
 };
}
// Reuse bounded caches and in-flight reads across sessions in this process.
export const sharedMarketDetailReader=createMarketDetailReader();
export async function runAdaptiveMarketDiscovery(body,intent,{discover=discoverMarketWithAI,readDetail=sharedMarketDetailReader,maxRounds=3,maxDetails=24,maxPages=6,concurrency=3,cachedListings=[],previouslyCheckedUrls=[],previouslyReadPages=[],onProgress=()=>{}}={}){
 ...
 if(!Array.isArray(previouslyCheckedUrls)||!Array.isArray(previouslyReadPages)||previouslyCheckedUrls.length>10000||previouslyReadPages.length>10000)throw Error('invalid-resume-history');
 const completedKeys=new Set(previouslyCheckedUrls.map(marketListingKey).filter(Boolean));
 const completedPages=new Set(previouslyReadPages.map(u=>marketDiscoveryPage(u)?.url).filter(Boolean));
 ...
 const enqueue=raw=>{const c=marketCandidate(raw);if(!c)return;const key=marketListingKey(c.url);if(discoveredKeys.has(key)||cachedKeys.has(key)||completedKeys.has(key))return;discoveredKeys.add(key);discoveredUrls.add(c.url);pending.set(c.url,c);};
 ...
 // One direct worker can run beside page discovery. A slow direct source
 // must not prevent a faster independent source from producing its first ad.
 const overlap=concurrency>1&&pending.size>0&&(found.discoveryPages||[]).some(u=>marketDiscoveryPage(u));
 const directWork=drain(Math.min(roundBudget,overlap?1:concurrency),overlap?1:concurrency);
 directWork.catch(()=>{});
 if(!overlap)await directWork;
 const pageWorkers=overlap?concurrency-1:concurrency;
 for(const raw of found.discoveryPages||[]){const page=marketDiscoveryPage(raw);if(page&&!seenPages.has(page.url)&&!completedPages.has(page.url))pendingPages.set(page.url,page);}
 ...
 while(pendingPages.size&&seenPages.size<pageAllowance){
  ...
  for(const page of pendingPages.values()){if(sources.has(page.source.id))continue;batch.push(page);sources.add(page.source.id);if(batch.length>=pageWorkers||seenPages.size+batch.length>=pageAllowance)break;}
  await mapBounded(batch,pageWorkers,async page=>{
   pendingPages.delete(page.url);seenPages.add(page.url);let pageResult;
   try{const html=await readDetail(page),links=detailLinksFromDiscoveryPage(html,page);for(const url of links)enqueue(url);const next=nextMarketDiscoveryPage(html,page);if(next&&!seenPages.has(next.url)&&!completedPages.has(next.url))pendingPages.set(next.url,next);pageResult={url:page.url,source:page.source.name,status:'expanded',detailLinks:links.length};}
   catch(e){
    if(/^HTTP (404|410)$/.test(e.message)){const fallback=marketDiscoveryFallback(page);if(fallback&&!seenPages.has(fallback.url)&&!completedPages.has(fallback.url))pendingPages.set(fallback.url,fallback);}
