...
import {extractHarajMileage} from './haraj-mileage.js';
...
export function parseHarajFastPage(html='',base='',opts={}){
  ...
  for(let i=0;i<raw.length;i++){
    ...
    const evidenceText=`${x.title} ${text.slice(0,2600)}`;
    const priceHit=extractHarajPrice(evidenceText,{year});
    const mileageHit=extractHarajMileage(evidenceText,{year});
    const price=priceHit?.price??null;
    const mileage=mileageHit?.mileage??null;
    if(f.minPrice&&(!price||price<Number(f.minPrice)))continue;
    if(f.maxPrice&&(!price||price>Number(f.maxPrice)))continue;
    if(f.minMileage&&(!mileage||mileage<Number(f.minMileage)))continue;
    if(f.maxMileage&&(!mileage||mileage>Number(f.maxMileage)))continue;
    const image=imageOf(seg,base);
    out.push({source:'Haraj',sourceType:'marketplace',seller:'Haraj',sourceStrict:true,title:x.title,snippet:text.slice(0,500),url:x.url,year,price,mileage,city,condition:'used',image:image||null,displayImage:image||null,imageVerified:Boolean(image),priceVerified:Boolean(priceHit),priceSource:priceHit?.source||null,priceEvidence:priceHit?.evidence||null,mileageVerified:Boolean(mileageHit),mileageSource:mileageHit?.source||null,mileageEvidence:mileageHit?.evidence||null,saleVerified:true,saleEvidence:['haraj_direct_ad_url','haraj_fast_search_page'],score:86,discovery:'haraj_fast_search_page',fastSource:true});
