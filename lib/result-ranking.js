...
function rankingContext(body){const query=String(body.query||'');return {catalog:catalogIntent(query),brand:detectRequestedBrand(query),year:exactYearIntent(query),requestedBody:normalizeBodyType(body.filters?.category)};}
export function relevanceScore(car={},body={},context=rankingContext(body)){
 const filters=body.filters||{}, {catalog,brand,year,requestedBody}=context;
 let score=0;
 ...
 if(requestedBody&&resolveBodyType(car)===requestedBody)score+=400;
...
export function rankListings(listings=[],body={}){
 const context=rankingContext(body);
 return listings.map((car,index)=>({car,index,score:relevanceScore(car,body,context)}))
