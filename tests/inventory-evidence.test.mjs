...
test('condition never comes from a requested tab, year, or zero/missing mileage',()=>{
 for(const mileage of [null,0])assert.equal(resolveCondition({sourceCategory:'cars for sale',source:'Haraj',condition:'new',title:'Camry 2026',mileage}),'unknown');
 assert.equal(resolveCondition({sourceCategory:'cars for sale',source:'Haraj',title:'كامري بحالة الوكالة بطارية جديدة',condition:'new'}),'unknown');
 assert.equal(resolveCondition({sourceCategory:'cars for sale',source:'Haraj',title:'سيارة جديدة',mileage:80000}),'used');
 assert.equal(resolveCondition({sourceCategory:'cars for sale',source:'Haraj',title:'Camry',sourceCondition:'new',mileage:0}),'new');
 assert.equal(resolveCondition({sourceCategory:'cars for sale',source:'Haraj',title:'كامري سيارة جديدة زيرو'}),'new');
});
test('unknown and used vehicles cannot enter New; new cannot enter Used',()=>{
 const base={sourceCategory:'cars for sale',source:'Haraj',title:'Toyota Camry 2023',year:2023,saleVerified:true};
...
test('canonical fields preserve existing contracts and source evidence',()=>{
 const c=normalizeInventoryListing({sourceCategory:'cars for sale',source:'Haraj',url:'https://haraj.com.sa/11111111',title:'Camry 2020',mileage:80000,price:45000,priceVerified:true,condition:'used',images:['https://example.org/car.jpg'],galleryVerified:true});
...
test('live Yaris RPM and financing warranty are not selling price or mileage',()=>{
 ...
 assert.equal(resolveCondition({sourceCategory:'cars for sale',source:'Haraj',description:'حالة السيارة : وكالة'}),'new');
});
...
test('All inventory includes unknown without admitting it to either condition tab',()=>{
 const car={sourceCategory:'cars for sale',source:'Haraj',title:'Toyota Camry 2023',year:2023,url:'https://haraj.com.sa/11111111',condition:'unknown',saleVerified:true};
...
for(const query of ['لاندكروزر ممشى أقل من 100 ألف','لاندكروزر ممشى أقل من ١٠٠ ألف','Land Cruiser mileage under 100k','Cars under 100,000 km'])test('mileage ceiling never becomes price: '+query,()=>{
 ...
 const base={sourceCategory:'cars for sale',source:'Haraj',condition:'used',saleVerified:true,title:'Toyota Land Cruiser 2020',brand:'Toyota',model:'Land Cruiser',year:2020};
...
test('explicit km suffix overrides an AI price misinterpretation',()=>{
 const query='Cars under 100,000 km',intent={...rulesIntent(query).intent,maxPrice:100000};
 const body=intentSearchBody({query,condition:'used',filters:{}},{intent,intentMode:'ai',searchQuery:'__all_cars__'});
 assert.equal(body.filters.maxMileage,100000);assert.equal(body.filters.maxPrice,undefined);
});
