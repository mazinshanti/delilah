...
import {marketCandidate,marketDiscoveryPage,marketListingKey,runAdaptiveMarketDiscovery,createMarketDetailReader} from './ai-market-discovery-trial.js';
...
export async function runDirectoryMarketSession(body,intent,{
 previous=null,taskBudget=null,detailBudget=60,searchConcurrency=2,
 discover=discoverDirectoryTask,readDetail=createMarketDetailReader(),onProgress=()=>{},save=()=>{}
}={}){
 ...
 const pending=new Map(report.pendingUrls.map(u=>[marketListingKey(u),u])),pages=new Set(report.pendingPages),readPages=new Set(report.readPages);
 ...
 function snapshot(){
  ...
  report.planComplete=completed.size===plan.length;report.accepted=cars.size;report.checked=checked.size;report.coverageComplete=false;
 ...
 async function validatePending(){
  ...
  const r=await runAdaptiveMarketDiscovery(body,intent,{maxRounds:1,maxDetails:detailBudget-checks,maxPages:2,
   previouslyCheckedUrls:report.results.map(r=>r.url),previouslyReadPages:[...readPages],readDetail,
 ...
 work=work.then(validatePending).then(()=>save(snapshot()));
