...
const publicComparable=car=>({url:car.url,originalUrl:car.originalUrl||car.url,title:car.title,make:car.make||car.brand,model:car.model,year:car.year,price:car.price,mileage:car.mileage??null,city:car.city,trim:car.trim||null,source:car.source,seller:car.seller||car.source,image:safeURL(car.image),lastSeenAt:car.lastSeenAt||null});
export function valueVehicle(records,vehicle,{snapshotAt=null,now=Date.now(),maxAgeMs=36*3600000,matchingTrims=[],aiMode='deterministic'}={}){
 ...
 // Without any exact-year evidence, never extrapolate a value from only newer or only older cars.
 if(!exactCount&&!(rows.some(c=>c.year<vehicle.year)&&rows.some(c=>c.year>vehicle.year)))return{...base,reason:'insufficient-year-evidence'};
 const learned=mileageAdjustment(candidates,vehicle);
 const knownMileage=rows.filter(c=>Number.isFinite(c.mileage));
 const closeMileageCount=rows.filter(c=>Number.isFinite(c.mileage)&&Math.abs(c.mileage-vehicle.mileageKm)<=Math.max(30000,vehicle.mileageKm*.35)).length;
 const mileageEvidenceWeak=knownMileage.length>0&&closeMileageCount<3;
 const weighted=rows.map(car=>{const mileageFit=Number.isFinite(car.mileage)?1/(1+Math.abs(car.mileage-vehicle.mileageKm)/30000)**2:.35;
 ...
 const confidence=vehicle.accidentHistory==='yes'||vehicle.vehicleCondition==='fair'||mileageEvidenceWeak?'limited':rows.length>=20&&exactCount/rows.length>=.8&&knownMileage.length/rows.length>=.8&&Object.keys(sources).length>=2&&spread<=.35?'high':rows.length>=7&&exactCount>=3&&spread<=.6?'medium':'limited';
 ...
 const representatives=[...weighted].sort((a,b)=>b.weight-a.weight).slice(0,6).map(({car})=>publicComparable(car));
 if(mileageEvidenceWeak)base.limitations.push('limited-mileage-evidence');
