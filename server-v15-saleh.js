...
const MODELS={
  corolla:['corolla','كورولا','كرولا','كورلا'],camry:['camry','كامري'],yaris:['yaris','يارس'],patrol:['patrol','باترول'],sunny:['sunny','صني'],wrangler:['wrangler','رانجلر'],
  tucson:['tucson','توسان'],elantra:['elantra','النترا','إلنترا'],sonata:['sonata','سوناتا'],accent:['accent','اكسنت','أكسنت'],sportage:['sportage','سبورتاج'],sorento:['sorento','سورينتو'],cerato:['cerato','سيراتو'],tahoe:['tahoe','تاهو'],
  'land cruiser':['land cruiser','لاند كروزر','لاندكروزر'],prado:['prado','برادو'],fortuner:['fortuner','فورتشنر'],rav4:['rav4','rav 4','راف 4','راف4'],veloz:['veloz','فيلوز'],
  preface:['preface','بريفيس','بريفايس','بريفاس'],eado:['eado','ايدو','إيدو'],coolray:['coolray','كولراي'],emgrand:['emgrand','امجراند','إمجراند'],monjaro:['monjaro','مونجارو'],
  cs35:['cs35','سي اس 35'],cs55:['cs55','سي اس 55'],cs75:['cs75','سي اس 75'],uni_k:['uni k','uni-k','يوني كي'],uni_v:['uni v','uni-v','يوني في']
};
...
const MODEL_BRAND={corolla:'toyota',camry:'toyota',yaris:'toyota','land cruiser':'toyota',prado:'toyota',fortuner:'toyota',rav4:'toyota',veloz:'toyota',patrol:'nissan',sunny:'nissan',wrangler:'jeep',tucson:'hyundai',elantra:'hyundai',sonata:'hyundai',accent:'hyundai',sportage:'kia',sorento:'kia',cerato:'kia',tahoe:'chevrolet',preface:'geely',coolray:'geely',emgrand:'geely',monjaro:'geely',eado:'changan',cs35:'changan',cs55:'changan',cs75:'changan',uni_k:'changan',uni_v:'changan'};
...
function salehId(url=''){try{return /^\/(?:en\/)?cars\/([a-f0-9]{24})(?:\/|$)/i.exec(new URL(url).pathname)?.[1]?.toLowerCase()||null}catch{return null}}
function hasAny(text,arr=[]){const t=norm(text);return arr.some(x=>t.includes(norm(x)))}
function priceFrom(text='',html=''){
  const t=digits(text).replace(/,/g,'');
  const vals=[...t.matchAll(/\b([1-9][0-9]{3,6})\s*(?:SAR|ر\.?س|ريال)\b/gi)].map(x=>Number(x[1]));
  const raw=digits(html).replace(/\\u0026/g,'&').replace(/,/g,'');
...
async function discoverSalehProducts(){
  ...
  while(queue.length&&seenSitemaps.size<30&&products.size<2500){const u=queue.shift();if(seenSitemaps.has(u))continue;seenSitemaps.add(u);try{const r=await fetchPage(u);for(const x of sitemapLocs(r.html)){const id=salehId(x);if(id){if(!products.has(id)||/\/en\/cars\//i.test(x))products.set(id,{url:x,title:'',id});continue}if(/sitemap/i.test(x)&&/^https?:\/\//i.test(x)&&!seenSitemaps.has(x))queue.push(x)}}catch(e){if(errors.length<8)errors.push(`sitemap:${e.message}`)}}
  try{const inventory=await fetchPage('https://www.salehcars.com/en/cars/all');for(const x of inventoryLinks(inventory.html,inventory.url))products.set(x.id,x)}catch(e){if(errors.length<8)errors.push(`inventory:${e.message}`)}
  const discoveredBeforeSeeds=products.size;
  for(const u of VERIFIED_SEEDS){const id=salehId(u);if(id&&!products.has(id))products.set(id,{url:u,title:'',id})}
  return{links:[...products.values()],discoveredBeforeSeeds,sitemapCount:seenSitemaps.size,seedCount:VERIFIED_SEEDS.length,errors};
}
function candidateScore(item,d){const s=decodeURIComponent(`${item.title||''} ${item.url||''}`);let score=0;if(d.model&&hasAny(s,MODELS[d.model]))score+=8;if(d.brand&&hasAny(s,BRANDS[d.brand]))score+=4;if(d.exact&&digits(s).includes(String(d.exact)))score+=3;return score}
async function scanSaleh(body={}){
  ...
  let links=discovery.links.map(x=>({...x,rank:candidateScore(x,d)})).filter(x=>(!d.model||hasAny(`${x.title} ${decodeURIComponent(x.url)}`,MODELS[d.model]))&&(!d.brand||hasAny(`${x.title} ${decodeURIComponent(x.url)}`,BRANDS[d.brand]))&&(!d.exact||digits(`${x.title} ${decodeURIComponent(x.url)}`).includes(String(d.exact))));
  if(!links.length)links=discovery.links.map(x=>({...x,rank:candidateScore(x,d)}));
  links.sort((a,b)=>b.rank-a.rank);links=links.slice(0,120);
  const out=[];
  for(let i=0;i<links.length;i+=8){const batch=await Promise.all(links.slice(i,i+8).map(async item=>{
    ...
    const matchText=`${title} ${decodeURIComponent(p.url)} ${text.slice(0,3000)}`;
    const ys=[...digits(matchText).matchAll(/\b(20\d{2})\b/g)].map(x=>Number(x[1]));const year=ys.find(y=>y>=2000&&y<=2035)||null;
...
function merge(a=[],b=[]){const map=new Map();for(const c of[...a,...b]){if(!c?.url)continue;const id=c.source==='Saleh Cars'?(c.salehProductId||salehId(c.url)):null;const k=id?`saleh:${id}`:String(c.url).replace(/[?#].*$/,'').replace(/\/$/,'');const prev=map.get(k);map.set(k,prev?{...prev,...c,score:Math.max(Number(prev.score)||0,Number(c.score)||0)}:c)}return[...map.values()]}
...
app.get('/api/health',async(req,res)=>{try{const r=await jinner('/api/health');res.status(r.status).json({...r.data,edge:'dalelah-v15-saleh',salehNativeInventory:true,salehPublicProductIndex:true,salehProductIdDedupe:true,productVersion:'1.5'})}catch(e){res.status(502).json({error:e?.message||String(e)})}});
