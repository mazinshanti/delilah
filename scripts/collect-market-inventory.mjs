import {refreshRetainedListings} from '../lib/retained-listing-refresh.js';
...
let confirmedRemovals=0,harajCursor=0,previousDiagnostics=[],crawlState={};
try{crawlState=JSON.parse(gunzipSync(await readFile('data/stock-crawl-state.json.gz')));}catch{}
try{const old=JSON.parse(gunzipSync(await readFile('data/market-inventory.json.gz')));previousDiagnostics=old.diagnostics||[];harajCursor=old.diagnostics?.find(d=>d.source==='Haraj')?.nextCursor||0;for(const r of old.listings||[])all.set(r.url,r);}catch{}
...
const merged=mergeHarajSnapshot([...all.values()].filter(c=>!haraj.removedUrls.includes(c.url)),haraj.listings);
all.clear();for(const record of merged)all.set(record.url,record);
diagnostics.push(haraj.diagnostics);
const retained=await refreshRetainedListings({records:[...all.values()],sources:SOURCE_REGISTRY.filter(s=>parsers[s.adapter]),parsers,get,state:crawlState.retainedDetails||{}});
for(const url of retained.removedUrls)all.delete(url);for(const row of retained.updates)all.set(row.url,row);
crawlState.retainedDetails=retained.state;diagnostics.push(...retained.diagnostics.map(d=>({...d,phase:'retained-detail-refresh',records:d.updated})));
if(!diagnostics.some(d=>d.records>0)&&!retained.removedUrls.length&&!haraj.removedUrls.length&&!confirmedRemovals){await atomicGzip('data/stock-crawl-state.json.gz',crawlState);throw new Error('No source successfully refreshed; preserving previous snapshot');}
