...
export function normalizeInventoryListing(input = {}, context = {}) {
  ...
  const mileageKm = input.mileageEvidenceConflict===true?null:numberOrNull(input.mileage_km ?? input.mileageKm ?? input.mileage ?? extractMileage([input.title,input.description,input.snippet].filter(Boolean).join(" ")));
  ...
  const condition = resolveCondition(input,{mileageKm});
