...
for(const [query,segment]of cases){
 ...
 const r=await runAdaptiveMarketDiscovery(body,understanding.intent,{discover,readDetail,maxRounds:2,maxDetails:24,onProgress:e=>{
  if(e.listing)console.log(JSON.stringify({stage:'accepted',query,source:e.source,url:e.url,title:e.listing.title,price:e.listing.price,mileage:e.listing.mileage}));
 ...
 await writeFile(output,JSON.stringify(report,null,2));console.log(JSON.stringify({stage:'case-complete',query,discovered:r.discovered,checked:r.checked,accepted:r.accepted,uniqueSessionAccepted:unique.size,totalMs:Date.now()-start}));
