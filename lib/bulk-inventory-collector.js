...
export async function collectBulkInventory({source,get,parse,previous={},state={},maxPages=500,wait=ms=>new Promise(r=>setTimeout(r,ms)),now=Date.now,onRecords=()=>{},onProgress=()=>{}}){
 ...
 try{
  ...
  for(let n=0;n<(source.id==='mercedes'?1:maxPages)&&now()-started<30*60000;n++){
   ...
   try{
    ...
    const html=await get(url.href),records=parse(html,source);successfulPages++;const added=[];
    ...
    if(!added.length){if(!records.length)errors.push({page,error:'no-parseable-records-or-source-blocked'});nextPage=1;ended=true;break;}
