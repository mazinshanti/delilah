...
export const SAUDI_MARKET_SEARCH_TARGETS=[...SOURCE_REGISTRY,...additions].filter((s,i,a)=>a.findIndex(x=>x.id===s.id)===i).map(s=>({id:s.id,name:s.name,url:s.url,host:new URL(s.url).hostname,status:s.status,evidence:s.evidence||s.reason}));
