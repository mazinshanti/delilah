...
export function createListingTrial({env=process.env,fetchImpl=fetch}={}){
 ...
 async function inspect(record,body={}){
  ...
  if(!review){
   ...
   try{review=await task;}catch{return {status:'ai-unavailable-or-invalid',listing:null};}
  }
  ...
  const baseline=accepted[0],conflicts=fields.filter(k=>review[k].value!==null&&(baseline[k]??(k==='make'?baseline.brand:null))!==review[k].value);
