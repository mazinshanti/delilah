...
async function get(url,timeout=4500){const r=await fetch(url,{redirect:'error',headers:{'User-Agent':'Dalelah/1.5 (+https://dalelah.co; vehicle-search-index)'},signal:AbortSignal.timeout(timeout)});if(!r.ok)throw Error(`HTTP ${r.status}`);const reader=r.body.getReader();let n=0,parts=[];while(true){const {done,value}=await reader.read();if(done)break;n+=value.length;if(n>4_000_000){await reader.cancel();throw Error('source-too-large');}parts.push(Buffer.from(value));}return Buffer.concat(parts).toString('utf8');}
export async function publicListingHtml(url,timeout=4500){
