...
export async function runAdaptiveMarketDiscovery(body,intent,{discover=discoverMarketWithAI,readDetail=createMarketDetailReader(),maxRounds=3,maxDetails=24,maxPages=6,concurrency=3,onProgress=()=>{}}={}){
 ...
 const started=performance.now(),feedback=[],seen=new Set(),pending=new Map(),discoveredUrls=new Set(),discoveredKeys=new Set(),seenPages=new Set(),pendingPages=new Map(),pageResults=[],results=[],listings=[];let webSearchCalls=0,firstResultMs=null,stopReason='round-budget';
 ...
 const enqueue=raw=>{const c=marketCandidate(raw);if(!c)return;const key=marketListingKey(c.url);if(discoveredKeys.has(key))return;discoveredKeys.add(key);discoveredUrls.add(c.url);pending.set(c.url,c);};
 ...
 const selected=[];
 while(selected.length<limit&&groups.some(g=>g.length))for(const group of groups){if(group.length&&selected.length<limit)selected.push(group.shift());}
 await mapBounded(selected,concurrency,async c=>{
 ...
 if(round>0&&!added&&!pending.size&&!(pendingPages.size&&seenPages.size<maxPages)){stopReason='no-new-urls';break;}
 }
