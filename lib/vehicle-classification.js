...
export function vehicleIdentity(car={}){
 const queryDerived=car.identityOrigin==='source-title'||['haraj_source_native_search','haraj_direct_volume_search','syarah_source_native_inventory','yallamotor_source_native_search'].includes(car.discovery);
 const declaredMake=queryDerived?null:catalogMake(car.make||car.brand||car.vehicle?.make||'');
 const declaredModel=queryDerived?null:car.model||car.vehicle?.model;
 ...
 const make=declaredMake?.name||title.make||(!queryDerived&&(car.schemaType==='Car'||/\/(?:cardetail|used-car)\//.test(car.url||''))?String(car.make||car.brand||'').trim()||null:null);
...
export function classifyVehicle(car={}){
 ...
 if(/PART/i.test(car.sourceVehicleKind||'')||PART.test(category))return result('VEHICLE_PART','parts_category');
 ...
 if(positiveNumber(car.mileage_km??car.mileageKm??car.mileage??extractMileage(`${title} ${description}`)))evidence.push('odometer');
