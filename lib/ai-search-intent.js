 ...
 async function provider(query){
  ...
  try{
   for(let attempt=0;;attempt++){
    ...
    const intent=normalizeAIIntent(JSON.parse(text),query);stats.successes++;stats.latencyMs+=clock()-started;
    ...
    return {intent,intentMode:'ai',model,aiLatencyMs:clock()-started};
   }
  }finally{active--;}
 }
 async function understand(query){
  const q=String(query||'').trim(),fallback=rulesIntent(q);
  const fail=reason=>{stats[fallback.intentMode]++;if(needsAI(q))stats.fallbacks++;return {...fallback,fallbackReason:reason,aiLatencyMs:0};};
...
export function intentSearchBody(incoming,result){
 ...
 if(i.bodyType)filters.category=i.bodyType;
...
export function applyIntentConstraints(rows,intent){
 ...
 return rows.filter(car=>{
  ...
  if(intent.originPreference&&origins.get(catalogText(make))!==intent.originPreference)return false;
  ...
  if(intent.bodyType&&catalogText(car.bodyType)===catalogText(intent.bodyType))signals.push('source-body-type-fit');
  if(intent.originPreference)signals.push('brand-origin-fit');
  ...
  return {...car,rankingSignals:signals,intentScore:signals.length};
