...
export async function discoverPublicCarListings({query,intent,apiKey,model="gpt-5.6-luna",timeout=6500,maxCandidates=8}){
  if(!apiKey||!query||intent?.partsRequested)return[];
  const prompt=`Find current PUBLIC Saudi Arabia whole-car sale listings matching this user's request: ${JSON.stringify(query)}\nStructured intent: ${JSON.stringify(intent||{})}\n${internetResearchRules()}\nSearch broadly across Saudi marketplaces, official dealers, certified-used sites and independent dealers. Return only direct listing URLs you actually found. Do not invent URLs. Keep the answer concise.`;
  const payload={model,input:[{role:"user",content:[{type:"input_text",text:prompt}]}],tools:[{type:"web_search"}],tool_choice:"required",include:["web_search_call.action.sources"],max_output_tokens:700};
  try{
    ...
    if(!r.ok)return[];const d=await r.json();const urls=urlsFromResponse(d).slice(0,maxCandidates*3),out=[];
