...
export class InventoryIndex{
 ...
 search(body={}){
  ...
  if(cached&&cached.until>now)return cached.rows.map(c=>({...c}));
  ...
  const candidates=choices.reduce((a,b)=>a.length<b.length?a:b).filter(c=>now-Date.parse(c.lastSeenAt||this.generatedAt)<this.maxAgeMs);
  ...
  // Cache only validated results; never extend a record beyond its freshness deadline.
  if(rows.length<=50000){while(this.searchCache.size>=32||this.cachedRows+rows.length>100000){const key=this.searchCache.keys().next().value;if(key===undefined)break;this.cachedRows-=this.searchCache.get(key).rows.length;this.searchCache.delete(key);}
   const until=rows.reduce((end,c)=>Math.min(end,Date.parse(c.lastSeenAt||this.generatedAt)+this.maxAgeMs),now+30000);this.searchCache.set(cacheKey,{until,rows});this.cachedRows+=rows.length;
  }
  return rows.map(c=>({...c}));
 }
 stats(){
