...
import {canonicalCity} from './city-normalization.js';
export {canonicalCity};
...
export function inventoryRecord(raw,source){
  ...
  const title=cleanText(raw.title);const candidate={...raw,title,url};
  recordClassification(source.name,classifyVehicle(candidate));
  if(!isVehicleSaleListing(candidate))return null;
  ...
  if(!make||!model||!year||!['new','used','unknown',null,undefined].includes(raw.condition))return null;
...
export function parseStructuredInventory(html,source){
  ...
  function walk(obj,parent={}){
    ...
    if(types.some(t=>['Car','Vehicle'].includes(t))){
      ...
      const r=inventoryRecord({schemaType:types.find(t=>['Car','Vehicle'].includes(t)),sourceCategory:obj.category,description,url,title:obj.name,make:obj.brand?.name||obj.brand,model:obj.model?.name||obj.model,year:obj.vehicleModelDate||obj.modelDate||obj.productionDate||obj.name?.match(/\b((?:19|20)\d{2})\b/)?.[1],trim,price:offers.price,mileage:obj.mileageFromOdometer?.value??obj.mileageFromOdometer,condition,city:city?city.replace(/-/g,' ').replace(/\b\w/g,c=>c.toUpperCase()):null,image:image?.url||image,images:obj.image,vin:obj.vehicleIdentificationNumber,bodyType:obj.bodyType,fuelType:obj.fuelType,transmission:obj.vehicleTransmission},source);
...
export function syarahHighResolutionImage(value){
  const url=safePublicUrl(value,'https://syarah.com');if(!url)return null;
  try{const u=new URL(url);if(u.hostname!=='cdn.syarah.com')return url;if(/^\/photos-thumbs\/online-v1\/0x(?:99|300|426)\//.test(u.pathname))u.pathname=u.pathname.replace(/^\/photos-thumbs\/online-v1\/0x(?:99|300|426)\//,'/photos-thumbs/online-v1/0x683/');return u.href}catch{return url}
}
export function parseSyarahInventory(html,source){
  ...
  return (data?.posts||[]).flatMap(p=>{
    ...
    const images=[p.image_url,...(Array.isArray(p.images_urls)?p.images_urls:[])].map(syarahHighResolutionImage).filter(Boolean);
    const r=inventoryRecord({url:p.product_url,title:p.title,make:g.post_make,model:g.post_model,year:g.post_year,trim:g.post_ext,price:p.sellingprice,mileage:g.post_mileage,condition:String(g.post_condition||'').toLowerCase(),city:g.post_city,image:images[0],images,fuelType:g.post_fuel,transmission:g.post_transmission,bodyType:g.post_body_type||g.post_body,color:g.post_color||null},source);
