...
const health=await fetch(`${base}/api/health`,{signal:timeout(10000)});if(!health.ok)throw new Error(`health ${health.status}`);const h=await health.json();if(!h.ok||h.edge!=='inventory-v20'||h.progressiveSearch!==true)throw new Error(`health config invalid: ${JSON.stringify(h)}`);console.log(`health ok; ${h.edge}; ${h.logic}`);
