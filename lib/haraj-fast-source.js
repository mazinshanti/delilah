import {attachHarajClassificationEvidence} from './haraj-classification-evidence.js';
...
const BAD=/(?:قطع\s*غيار|مكين[هة]|محرك|ايرباق|ارباق|طبلون|كمبروسر|دينمو|رديتر|صدام|شبك|شمعة|شمعات|انوار|أنوار|رفرف|كبوت|جنط|جنوط|كفرات|فلتر|طرمب[هة]|حساس|اصطب|اسطب|كشاف|مراي[هة]|مرآة|تشليح|للتشليح|للايجار|للإيجار|تاجير|تأجير|ورشة|صيانة|مطلوب|شراء سيارات|عداد.{0,60}(?:اصلي|أصلي|وكاله|وكالة|يركب|تركيب))/i;
...
function yearsOf(s=''){return [...new Set([...digits(s).matchAll(/\b((?:19|20)\d{2})\b/g)].map(x=>Number(x[1])).filter(y=>y>=1980&&y<=2035))]}
...
export function localizedQuery(query=''){const d=intent(query);if(d.model)return [HARAJ_ARABIC[d.model],d.exact].filter(Boolean).join(' ');const make=catalogMake(query);let base=String(query).trim();if(make?.ar&&make.ar!==make.name)base=base.replace(new RegExp(make.name.replace(/[.*+?^${}()|[\]\\]/g,'\\$&'),'i'),make.ar);return base;}
export function parseHarajFastPage(html='',base='',opts={}){
  ...
  for(let i=0;i<raw.length;i++){
    ...
    if(f.maxMileage&&(extractMileage(text)==null||extractMileage(text)>Number(f.maxMileage)))continue;
    const image=imageOf(seg,base);
    out.push({source:'Haraj',sourceType:'marketplace',seller:'Haraj',sourceStrict:true,title:x.title,snippet:text.slice(0,500),url:x.url,year,price,mileage:extractMileage(text),city,condition:'used',image:image||null,displayImage:image||null,imageVerified:Boolean(image),priceVerified:Boolean(priceHit),priceSource:priceHit?.source||null,priceEvidence:priceHit?.evidence||null,saleVerified:true,saleEvidence:['haraj_direct_ad_url','haraj_fast_search_page'],score:86,discovery:'haraj_fast_search_page',fastSource:true});
    if(out.length>=24)break;
  }
  return attachHarajClassificationEvidence(out,html);
}
