...
export function createSources(transport,{webDiscovery=null,selectedSources=null}={}){
 ...
 async function verify(c,{signal}){
  if(['OpenSooq','Mercedes-Benz Saudi'].includes(c.source)){const source=c.source==='OpenSooq'?sources.opensooq:sources.mercedes;const p=await sourceGet(transport,c.url,signal,1);return schemaCars(p.text,source,{detailUrl:p.url});}
