...
export function applyIntentConstraints(rows,intent){
 ...
 return rows.filter(car=>{
  ...
  const body=catalogText(car.bodyType||car.vehicle?.bodyType||'');
  if(intent.bodyType&&body&&body!==catalogText(intent.bodyType))return false;
  ...
  if(intent.bodyType&&!car.bodyType&&!car.vehicle?.bodyType)unverifiedAttributes.push('bodyType');
  if(intent.originPreference&&!origin)unverifiedAttributes.push('brandOrigin');
  return {...car,rankingSignals:signals,intentScore:signals.length,unverifiedAttributes};
