...
const knownDirect=[
  ...
  u=>/arabwheels\.sa$/i.test(u.hostname)&&/\/used-cars\/.+\d+/i.test(u.pathname),
  u=>/yallamotor\.com$/i.test(u.hostname)&&/\/used-cars\/.+/i.test(u.pathname),
  u=>/carswitch\.com$/i.test(u.hostname)&&/\/used-cars\/.+/i.test(u.pathname)&&!/-price(?:\/|$)/i.test(u.pathname),
  u=>/motory\.com$/i.test(u.hostname)&&/\/used-cars\/.+/i.test(u.pathname),
  u=>/salehcars\.com$/i.test(u.hostname)&&/\/cars?\/.+/i.test(u.pathname)
];
...
function directEnough(u,title,text){if(BLOCK_PATH.test(u.pathname))return false;if(WANTED.test(`${title} ${text}`)||PART_WORDS.test(`${title} ${text}`))return false;if(knownDirect.some(fn=>fn(u)))return true;const seg=u.pathname.split("/").filter(Boolean);return seg.length>=2&&SALE_WORDS.test(`${title} ${text}`)&&Boolean(yearFrom(`${title} ${text}`)||priceFrom(`${title} ${text}`));}
...
export async function discoverPublicCarListings({query,intent,apiKey,model="gpt-5.6-luna",timeout=7500,maxCandidates=8}){
  if(!apiKey||!query||intent?.partsRequested)return[];
  const prompt=`Act as Dalelah's live Saudi car discovery worker. Find current PUBLIC whole-car sale listings matching: ${JSON.stringify(query)}\nIntent: ${JSON.stringify(intent||{})}\n${internetResearchRules()}\nUse several focused web searches. Prioritize haraj.com.sa, syarah.com, saudisale.com, arabwheels.sa, yallamotor.com, carswitch.com, motory.com, official Saudi dealer inventory and public independent-dealer pages. Search for exact make/model/year/city when provided. Return direct listing pages only; never category/search pages or invented URLs.`;
  const payload={model,reasoning:{effort:"none"},input:[{role:"user",content:[{type:"input_text",text:prompt}]}],tools:[{type:"web_search_preview",search_context_size:"medium",user_location:{type:"approximate",country:"SA",city:intent?.city||"Riyadh",timezone:"Asia/Riyadh"}}],tool_choice:"auto",include:["web_search_call.results","web_search_call.action.sources"],max_output_tokens:900};
  try{
    ...
    console.log("WEB_DISCOVERY_TRACE",JSON.stringify({query,status:r.status,urls:urls.length,hosts:seenHosts,accepted:out.length,acceptedUrls:out.map(x=>x.url).slice(0,5)}));
    return out;
