...
for(const [text,price,source] of cases){
  test(`Syarah cash price: ${text.slice(0,48)}`,()=>{
    ...
    assert.ok(hit);
    assert.equal(hit.price,price);
    assert.equal(hit.source,source);
    assert.equal(hit.confidence,'high');
  });
}

test('does not use installment as vehicle price',()=>{
  assert.equal(extractSyarahCashPrice('Installment 2,451 SAR/month Used 47,262 KM'),null);
});

test('does not use discount or previous price without cash label',()=>{
  assert.equal(extractSyarahCashPrice('Coupon 1,500 SAR OFF discount 18,600 SAR previous 133,500 SAR'),null);
});

test('does not use mileage as price',()=>{
  assert.equal(extractSyarahCashPrice('Used 138,153 KM Inspected & Guaranteed'),null);
});
