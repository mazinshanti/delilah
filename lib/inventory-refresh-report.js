...
export function inventoryRefreshReport(snapshot,state={},now=Date.now()){
 const rows=snapshot.listings||[],accepted=filterVehicleSaleListings(rows.map(c=>normalizeInventoryListing(c))),active=accepted.filter(c=>!isInactiveListing(c)),fresh=active.filter(c=>{const at=Date.parse(c.lastSeenAt);return Number.isFinite(at)&&at<=now&&now-at<36*3600000;});
