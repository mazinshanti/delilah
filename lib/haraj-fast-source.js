...
const BAD=/(?:قطع\s*غيار|مكين[هة]|محرك|ايرباق|ارباق|طبلون|كمبروسر|دينمو|رديتر|صدام|شبك|شمعة|شمعات|انوار|أنوار|رفرف|كبوت|جنوط|كفرات|فلتر|طرمب[هة]|حساس|اصطب|اسطب|كشاف|مراي[هة]|مرآة|تشليح|للتشليح|للايجار|للإيجار|تاجير|تأجير|ورشة|صيانة|مطلوب|شراء سيارات)/i;
...
function yearOf(s=''){const ys=[...digits(s).matchAll(/\b((?:19|20)\d{2})\b/g)].map(x=>Number(x[1])).filter(y=>y>=1980&&y<=2035);return ys[0]||null}
...
export function parseHarajFastPage(html='',base='',opts={}){
  ...
  for(let i=0;i<raw.length;i++){
    ...
    const year=yearOf(`${x.title} ${text.slice(0,500)}`);
