...
export function extractMileage(text=''){
 const s=String(text).replace(/[٠-٩]/g,d=>'٠١٢٣٤٥٦٧٨٩'.indexOf(d)).replace(/[٬,]/g,'').replace(/٫/g,'.');
 if(/\d+\s*[-–]\s*\d+\s*(?:km|كم|كلم)/i.test(s))return null;
 const label=/(?:mileage|odometer|الممشى|الممشي|ممشى|ممشي|العداد|عداد)\s*[:：]?\s*(\d+(?:\.\d+)?)\s*(ألف|الف|آلاف|thousand|k\b)?\s*(km\b|كلم|كم)?/i.exec(s);
 const unit=/(?:^|\s)(\d+(?:\.\d+)?)\s*(ألف|الف|آلاف|thousand|k\b)?\s*(km\b|كلم|كم)(?:\s|$|[.,])/i.exec(s);
