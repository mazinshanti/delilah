...
const legacyPort=Number(process.env.DALELAH_LEGACY_PORT||7600);
...
async function inner(path,opts={}){
  const response=await fetch(`http://127.0.0.1:${legacyPort}${path}`,{...opts,signal:opts.signal||AbortSignal.timeout(55_000)});
...
function publicJob(job){
  ...
  const out={...base,listings,counts:counts(listings),complete,marketScanComplete:complete,directCoreLane:true,directCoreFirstResultMs:job.firstResultMs??null,directCoreDurationMs:direct.durationMs??null,directCoreSources:direct.sources||[],directCoreErrors:direct.errors||[],legacyScanStarted:Boolean(job.fullPromise),exactBrandQualityGate:true};
...
function kickFull(job){
  ...
  job.fullPromise=inner('/api/search',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(job.body),signal:AbortSignal.timeout(55_000)})
    .then(({response,data})=>{
      job.fullDone=true;
...
async function advanceFull(job){
  ...
  if(!upstream)return;
  try{
    const {response,data}=await inner(`/api/search/progress/${encodeURIComponent(upstream)}`,{signal:AbortSignal.timeout(50_000)});
...
app.post('/api/search',async(req,res)=>{
  const body=req.body||{};
  if(isBroad(body)){
    try{const {response,data}=await inner('/api/search',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(body)});return res.status(response.status).json(data);}catch(error){return res.status(502).json({error:error?.message||'Dalelah search unavailable'});}
  }
...
app.get('/api/search/progress/:id',async(req,res)=>{
  ...
  if(!id.startsWith('dc.')){
    try{const {response,data}=await inner(`/api/search/progress/${encodeURIComponent(id)}`);return res.status(response.status).json(data);}catch(error){return res.status(502).json({error:error?.message||'Dalelah progress unavailable'});}
  }
...
app.get('/api/health',async(_req,res)=>{
  try{const {response,data}=await inner('/api/health',{signal:AbortSignal.timeout(9000)});return res.status(response.status).json({...data,directCoreLane:true,directCoreStrategy:'direct-first-legacy-deep-scan',directCoreBudgetMs:DIRECT_BUDGET_MS,directCoreFullHeadStartMs:FULL_HEAD_START_MS,directCoreJobs:jobs.size});}
  catch(error){return res.status(503).json({ok:false,directCoreLane:true,error:error?.message||String(error)});}
});
...
async function proxy(req,res){
  try{
    ...
    const response=await fetch(`http://127.0.0.1:${legacyPort}${req.originalUrl}`,{method:req.method,headers,body,redirect:'manual',signal:AbortSignal.timeout(55_000)});
...
app.listen(externalPort,()=>console.log(`Dalelah direct-core candidate on ${externalPort}; legacy deep scan ${legacyPort}`));
