import {parseOpenSooqDetail} from '../opensooq-detail.js';
import {openSooqSearchUrl} from '../opensooq-source.js';
import {sourceGet} from './expanded-sources.mjs';
import {ORIGINS} from '../../lib/ai-search-intent.js';
import {resolveBodyType} from '../../lib/vehicle-body-type.js';
import {additionalDiscovery,additionalVerify,isAdditionalSource} from './expanded-sources.mjs';
...
import {catalogIntent,VEHICLE_CATALOG} from '../../public/catalog.js';
...
export function schemaCars(html,source,{detailUrl=null}={}){
 const out=[];function walk(node,parent={}){if(!node||typeof node!=='object')return;
 const types=[node['@type']].flat();
 if(types.some(t=>t==='Car'||t==='Vehicle')){
  const url=node.url||parent.url;let absolute;try{absolute=new URL(url,source.url).href;}catch{return;}
  if(!identity(absolute)||detailUrl&&identity(absolute)!==identity(detailUrl))return;
  const offer=[node.offers||parent].flat()[0]||{};
 ...
 for(const v of Object.values(node))if(v&&typeof v==='object')Array.isArray(v)?v.forEach(x=>walk(x,node)):walk(v,node);
 }
...
export function planQueries(i){
 ...
 const seeds=i.bodyType==='SUV'?['Toyota Fortuner','Nissan X-Trail','Mazda CX-9','Honda CR-V','Mitsubishi Pajero','Lexus RX','Hyundai Tucson','Kia Sportage','Ford Explorer','BMW X5']:['Toyota Corolla','Nissan Sunny','Honda Accord','Mazda 6','Mitsubishi Lancer','Lexus ES','Hyundai Elantra','Kia K5','Ford Taurus','BMW 3 Series'];
 const valid=q=>{const c=catalogIntent(q);return c.make&&c.model&&(!i.bodyType||resolveBodyType({make:c.make,model:c.model})===i.bodyType)&&(!i.originPreference||ORIGINS[i.originPreference]?.includes(c.make))&&!i.excludedMakes.includes(c.make);};
 const pool=seeds.filter(valid),seen=new Set(pool.map(q=>catalogIntent(q).make));
 for(const make of VEHICLE_CATALOG.makes){if(seen.has(make.name))continue;const model=make.models.find(m=>valid(make.name+' '+m.name));if(model){pool.push(make.name+' '+model.name);seen.add(make.name);}}
 return pool.slice(0,6);
}
export function createSources(transport,{webDiscovery=null,selectedSources=null}={}){
 async function discover(i,{signal,onBatch,diagnostics}){
  const queries=planQueries(i),jobs=[additionalDiscovery(i,{transport,signal,onBatch,diagnostics,selectedSources})];
  const schedule=(source,url,parse)=> (!selectedSources||selectedSources.includes(source))&&jobs.push((async()=>{try{const page=await transport.get(url,{signal});const rows=parse(page.text,page.url);onBatch(rows,source);diagnostics.push({stage:'discovery',source,candidates:rows.length,url});}catch(e){diagnostics.push({stage:'discovery',source,url,error:e.message});}})());
  ...
  if(i.condition!=='new')for(const q of queries.slice(0,2)){
   if(!selectedSources||selectedSources.includes('OpenSooq'))jobs.push((async()=>{const source=sources.opensooq,url=openSooqSearchUrl({query:q,filters:{city:i.city}});try{const p=await sourceGet(transport,url,signal);const rows=schemaCars(p.text,source);onBatch(rows,source.name);diagnostics.push({stage:'discovery',source:source.name,candidates:rows.length,url});}catch(e){diagnostics.push({stage:'discovery',source:source.name,url,error:e.message});}})());
  }
  if(i.condition!=='new'&&(!i.make||i.make==='Mercedes')&&(!selectedSources||selectedSources.includes('Mercedes-Benz Saudi')))jobs.push((async()=>{const source=sources.mercedes,url=new URL(source.path,source.url).href;try{const p=await sourceGet(transport,url,signal);const rows=schemaCars(p.text,source);onBatch(rows,source.name);diagnostics.push({stage:'discovery',source:source.name,candidates:rows.length,url});}catch(e){diagnostics.push({stage:'discovery',source:source.name,url,error:e.message});}})());
  if(i.condition==='new'){
  ...
  for(const q of (selectedSources&&!selectedSources.includes('Syarah')?[]:queries)){const c=catalogIntent(q),path=[c.make,c.model].filter(Boolean).map(s=>s.toLowerCase().replace(/ /g,'-')).join('/');
 ...
 async function verify(c,{signal}){
  if(['OpenSooq','Mercedes-Benz Saudi'].includes(c.source)){const source=c.source==='OpenSooq'?sources.opensooq:sources.mercedes;const p=await sourceGet(transport,c.url,signal,1);return c.source==='OpenSooq'?parseOpenSooqDetail(p.text,p.url,source):schemaCars(p.text,source,{detailUrl:p.url});}
  if(isAdditionalSource(c.source))return additionalVerify(c,{transport,signal});
  const fetchUrl=c.source==='Haraj'?`https://haraj.com.sa/${new URL(c.url).pathname.split('/')[1]}/`:c.url;
  const page=await transport.get(fetchUrl,{signal,priority:1});
